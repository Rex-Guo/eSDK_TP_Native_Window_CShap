...
{
    public partial class DeleteSitesList : Form
    {
        public string[] sites_URI;
        public DeleteSitesList()
        {
            InitializeComponent();
        }
        ...
        private void OK_Button_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.lb_SiteURI.Items.Count != 0)
                {
                    List<string> list = new List<string>();
                    for (int i = 0; i < this.lb_SiteURI.Items.Count; i++)
                    {
                        list.Add(this.lb_SiteURI.Items[i].ToString());
                    }
                    this.sites_URI = list.ToArray();
                }
                else
                {
